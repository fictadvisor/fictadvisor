...
const SuperheroForm = ({ authentication, onBack }: SuperheroFormProperties) => {
  const { error, isLoading, mutate, isSuccess } = useMutation((data: CreateSuperheroBody) => api.superheroes.createSuperhero(authentication.getToken(), data));
  ...
  const [username, setUsername] = useState(authentication.user.username ? `@${authentication.user.username}` : '');
