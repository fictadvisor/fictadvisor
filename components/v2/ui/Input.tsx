...
const Input: React.FC<InputProps> = (props) => {
    ...
    const className = mergeClassNames(styles[inputColor], styles[inputStyle], styles[additionalClass]);


    return (
        <div className={className}>
            <label > {props.label} </label>
            {leftIcon && <div className={`${styles["icon"]} ${styles["left-icon"]}`}>{leftIcon}</div>}
            {rightIcon && <div className={`${styles["icon"]} ${styles["right-icon"]}`} onClick={handleIconClick}>{rightIcon}</div>}
