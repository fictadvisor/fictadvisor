...
const ReviewList = ({ data, isLoading, isFetching, page, setPage }) => {
  if (data.count === 0) {
    return (
      <Disclaimer>На жаль, у нас немає відгуків про цього викладача</Disclaimer>
    );
  }
...
const ReviewBlock = ({ link, className, ...props }: ReviewBlockProperties) => {
  ...
  return (
    <div>
      <div
        className={mergeClassName("d-flex adaptive-input-container", className)}
        style={{ marginBottom: "10px" }}
        {...props}
      >
        <SearchInput
          className="d-flex-grow"
          placeholder="Пошук відгуків за предметом"
          value={searchText}
