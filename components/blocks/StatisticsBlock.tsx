...
const EntryList = ({ data }) => {
  if (data.items.length === 0) {
    return (
      <Disclaimer>На жаль, у нас немає статистики цього викладача</Disclaimer>
    );
  }

  const average = +(
    data.items.reduce((p, c) => p + c.value, 0) / data.items.length
  ).toFixed(2);

  return (
    <>
      <Disclaimer className="m-b">
        Використовується статистика, яка була зібрана командою{" "}
        <a className="f-bold">{config.contacts.sovaChannelName}</a> у 2020 році
      </Disclaimer>
      <div
        className="statistics-field-group block"
        style={{ borderRadius: "8px 8px 0 0" }}
      >
        {data.items.map((e, i) => (
          <StatisticsEntry key={i} name={e.name} value={e.value} />
        ))}
