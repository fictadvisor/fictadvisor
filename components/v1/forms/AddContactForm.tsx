...
const AddContactForm = ({authentication, link, onBack}: AddContactFormProperties) => {
    const {
        ...
        mutate,
        isSuccess
    } = useMutation((data: CreateContactBody) => api.teachers.createContact(authentication.getToken(), link, data));
        ...
        <div className="form-block">
            <div className="block">
                ...
                <div className="d-flex">
                    ...
                    <Button loading={isLoading} className="d-flex-grow w-full m-l"
                            onClick={() => onSubmit()}>Відправити</Button>
                </div>
            </div>
            {
                validationErrors && validationErrors.length > 0
                    ...
                    <ErrorMessage className="m-t"
                                  text={(error as any)?.response?.status === 409 ? 'Контакт вже наявний у списку' : null}
                                  error={error}/>
            }
        </div>
    )
