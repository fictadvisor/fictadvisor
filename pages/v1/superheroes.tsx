...
const SuperheroesPage = () => {
  ...
  return (
    <PageLayout title="Наші супергерої">
      <p>
        Супергерой — це студент, який допомогає абітурієнтам та молодшим курсам
        з їхніми питаннями та проблемами.
        <br />
        Якщо ти потребуєш допомоги, не соромся їм писати.
      </p>
      <p>
        У разі некоректної поведінки супергероя, будь ласка, напишіть нам до
        бота зворотного зв'язку:{" "}
        <a href={`https://t.me/${config.contacts.feedbackBot}`} target="_blank">
          @{config.contacts.feedbackBot}
        </a>
      </p>
      {superheroData ? (
        <Disclaimer
          className={superheroData.state === "hidden" ? "alert" : null}
        >
          {superheroStateMessage[superheroData.state]}
        </Disclaimer>
      ) : formMode ? (
        <SuperheroForm
