...
const Rating = ({ rating }) => {
  return rating && rating > 0 ? (
    <div className="rating c-secondary" style={{ paddingTop: "1px" }}>
      {+rating.toFixed(2)}
      <StarIcon />
    </div>
  ) : null;
...
const TeacherPage = ({ teacher }) => {
  ...
  return (
    <PageLayout meta={{ title: fullName }} title="Сторінка викладача">
      {StateMessage && <StateMessage />}
      <div
        className={`block teacher ${collapsed ? "collapsed" : ""} ${
      ...
      {canCollapse && (
        <Button className="attached" onClick={() => setCollapsed(!collapsed)}>
          <span className={collapsed ? null : "t-v-flipped"}>
            <ArrowIcon />
          </span>
        </Button>
      )}
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  try {
    const data = await api.teachers.get(
