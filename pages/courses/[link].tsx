...
const ReviewList = ({ data, isFetching, setPage, page }) => {
  if (data.count === 0) {
    return <Disclaimer>На жаль, у нас немає відгуків про цей курс</Disclaimer>;
  }
  ...
  return (
    <>
      <div className="review-group">
        {data.items.map((r, i) => (
          <Review key={i} rating={r.rating} content={r.content} date={r.date} />
        ))}
      </div>
...
const CoursePage = ({ course }) => {
  ...
  const { data, isLoading, isFetching, error } = useQuery(
    ...
    () =>
      api.courses.getReviews(course.link, {
        page: 0,
    ...
    >
      ...
      <Link href={`/teachers/${course.teacher.link}`}>
        <a className="simple">
          <div className="block m-b d-flex">
            <div className="d-flex-grow f-bold" style={{ marginTop: "2px" }}>
              {fullName}
            </div>
            <div style={{ margin: "-6px 0 -12px" }}>
              <img
                className="avatar teacher small"
      ...
      <div className="adaptive-input-container d-flex m-t m-b">
        <SearchInput
          ...
          placeholder="Пошук відгуку за змістом"
          value={searchText}
