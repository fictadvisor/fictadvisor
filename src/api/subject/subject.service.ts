...
export class SubjectService {
    ...
    async getSubjects(query: SearchableQueryDto): Promise<Page<SubjectItemDto>> {
        const [items, count] = await this.subjectSearchIndexRepository.findAndCount({
            ...
            where: {
                state: !query.all ? SubjectState.APPROVED : Not(SubjectState.DECLINED),
    ...
    private async getSubjectById(id: string): Promise<Subject> {
        ...
        if (subject == null) {
            throw ServiceException.create(HttpStatus.NOT_FOUND, 'Subject with given id not found');
        }
    ...
    async addSubject(subject: SubjectCreateDto, user: User): Promise<SubjectDto> {
        const existing = await this.subjectRepository.findOne({ link: subject.link() });

        if (existing) {
            throw ServiceException.create(HttpStatus.CONFLICT, "Subject already exists");
        }

        try {
            const inserted = await this.subjectRepository.save(assign(
                new Subject(),
                {
                    link: subject.link(),
                    name: subject.name,
            ...
            this.telegramService.broadcastPendingSubject(user, inserted)
                .catch(e => this.logger.error('Failed to broadcast a pending subject', {
                    subject: inserted.id,
                    error: e.toString()
                }));
            ...
            throw ServiceException.create(HttpStatus.INTERNAL_SERVER_ERROR, "Failed to save subject")
        }
    }

    async updateSubject(id: string, update: SubjectUpdateDto): Promise<SubjectDto> {
        const subject = await this.getSubjectById(id);

        if (update.name != null) { subject.name = update.name }
        if (update.state != null) { subject.state = update.state; }
        if (update.description != null) { subject.description = update.description; }
