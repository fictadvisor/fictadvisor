...
const Dropdowns: FC<DropdownsProps> = ({
  ...
  function editSelective(newValue: string, oldValue: DropDownOption) {
    // change connected and disconnected selectives
    ...
    // change available options accordingly
  ...
  function handleSelectiveAdd(newValue: string) {
    // add selective to picked
    ...
    // add selective to connected
    setConnectedSelective(prev => {
      return [...prev, newValue];
    });
    // remove it from available options
  ...
  function handleSelectiveDelete(value: DropDownOption) {
    // remove selective from picked
    setPickedSelectives(selectives =>
      selectives.filter(selective => selective.id !== value.id),
    );
    // remove selective from connected
      ...
      {!!availableAmount &&
        (!isAddingSelective ? (
          ...
          <Dropdown
            ...
            options={selectiveOptions}
            showRemark={false}
            onChange={(value: string) => handleSelectiveAdd(value)}
            value={''}
            label=""
