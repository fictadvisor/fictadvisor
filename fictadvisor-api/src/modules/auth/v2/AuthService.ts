...
import { PrismaService } from '../../../database/v2/PrismaService';
...
import { InvalidResetTokenException } from '../../../common/exceptions/InvalidResetTokenException';
import { TooManyActionsException } from '../../../common/exceptions/TooManyActionsException';
import { InvalidVerificationTokenException } from '../../../common/exceptions/InvalidVerificationTokenException';
...
import { NotRegisteredException } from '../../../common/exceptions/NotRegisteredException';
import { PasswordRepeatException } from '../../../common/exceptions/PasswordRepeatException';
...
export class AuthService {
  ...
  async pseudoRegister (user: UserDTO, createStudent: Omit<StudentDTO, 'isCaptain'>) {
    const dbUser = await this.userRepository.updateById(
      { email: user.email }, {
        ...user,
        lastPasswordChanged: new Date(),
        state: State.APPROVED,
      });
    await this.studentRepository.updateById({ userId: dbUser.id }, createStudent);
